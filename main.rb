  ...
  def solve_mod_prime_power(matrix, rhs, num_vars, prime, modulus)
    ...
    (0...num_vars).each do |col|
      ...
      (pivot_row...num_rows).each do |r|
        # У кільці Z/(q^e)Z оберненими є тільки елементи, які не діляться на q!!!!!!!!!!!
...
class IndexCalculusSequential
  ...
  def solve
    ...
    puts 'mode = sequential' if @verbose
    ...
    DEFAULT_MAX_SOLVE_ATTEMPTS.times do
      ...
      puts "collecting relations: need #{additional} more..." if @verbose

      collect_relations(additional).each do |exponents, k|
        matrix << exponents
        rhs << k
      end
  ...
  def self.solve_once(alpha:, beta:, p:, verbose: true)
    solver = new(alpha: alpha, beta: beta, p: p, verbose: verbose)
    result = nil
...
if $PROGRAM_NAME == __FILE__
    ...
    print "p: "
    p = gets.strip.to_i
    result = IndexCalculusSequential.solve_once(alpha: alpha, beta: beta, p: p, verbose: true)
    puts "x = #{result[:x]}"
    puts "time: #{format('%.6f', result[:time_sec])} s"
end
